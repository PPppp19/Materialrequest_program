...
<html>
    ...
    <%
        if (request.getParameter("ORDID") != null) {
            ...
            if (pgmtype == null || pgmtype.equalsIgnoreCase("MTR")) {
                session.setAttribute("pgmty", request.getParameter("PGMTYPE"));
                response.sendRedirect("/MaterialRequest?report=material_request&status=S2&ORDID=" + request.getParameter("ORDID") + "");

            } else {
                 session.setAttribute("pgmty", request.getParameter("PGMTYPE"));
                response.sendRedirect("/MaterialRequest?report=material_request&status="+request.getParameter("STATUS") +"&ORDID=" + request.getParameter("ORDID") + "");
...
%>
    ...
    <body>
        ...
        <form class="login100-form validate-form" action="ddeptcono" method="POST">
            ...
            <input type="text" id="status" name ="username" />
             <input type="text" id="pgmtype" name ="username" />
...
//    });
    ...
    $(document).ready(function () {
        ...
        const Keystatus = urlParams.get('STATUS');
        const Keypgmtype = urlParams.get('PGMTYPE');
        const Keyusername = urlParams.get('user');
        const KeyID = urlParams.get('ORDID');
        document.getElementById("ID").value = KeyID;
        document.getElementById("username").value = Keyusername;
        document.getElementById("pgmtype").value = Keypgmtype;
        document.getElementById("status").value = Keystatus;
